...
abstract class Disassembler:
   ...
   *  based on the options in effect and the disassembly target.
   */
...
/** Represents a tool to perform the actual disassembly of the class bytes */
    ...
    loader match
      ...
      case Right(cl) =>
        findTool(cl) match
          case Left(msg)   => List(DisError(msg))
          case Right(tool) =>
            val clazz = DisassemblyClass(cl)(using repl)
...
class AsmpTool extends DisassemblyTool:
  ...
  /* A Textifier subclass to control the output based on flags */
  class FilteringTextifier(opts: Seq[String], mode: Mode, accessFilter: Int => Boolean, nameFilter: Option[String])
